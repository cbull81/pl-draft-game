...
def print_model_inputs(xi_df: pd.DataFrame, result: dict) -> None:
    ...
    eff_games = result.get("effective_games", 38.0)
    penalty_note = "" if avg_games >= 30 else f"  ← penalty (avg {avg_games:.1f} < 30)"
    pv = bd.get("position_value", {})
    ...
    print(f"  Position value (age-adj, position-normalised):")
    label_map = {"gk_value_z": "GK", "def_value_z": "DEF", "mid_value_z": "MID", "fwd_value_z": "FWD"}
    for k, label in label_map.items():
        print(f"    {label}: {pv.get(k, 0):+.3f}")
    print(f"  ppg_hat  = Stage 1 [xGF, xGA, GK/DEF/MID/FWD val_z]  →  {bd['ppg_hat']:.4f}")
    print(f"  avg games = XI average games played           →  {avg_games:.1f} / 38{penalty_note}")
    print(f"  pts       = ppg × {eff_games:.0f}                        →  {bd['ppg_hat'] * eff_games:.1f}")
    print("  " + "─" * 54)
...
def play():
    ...
    avg_games = result.get("avg_games", 38.0)
    penalty_str = f"  (penalty — avg {avg_games:.1f} games)" if avg_games < 30 else ""
    print(f"\n  Predicted points:  {result['predicted_points']:.1f} / 114{penalty_str}")
